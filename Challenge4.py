...
def number(string):
    ...
    return number
'''
def value(string,search):
    if search == "hcl":
        temp = string[string.find(search)+4 : string.find(search)+10]
        for x in temp:
            if(x not in String.hexdigits):
                return False
        return True
    elif search == "ecl":
        temp = string[string.find(search)+4 : string.find(search)+7]
        ...
        #maybe not working
        temp = number(string[string.find(search)+4:])
        num = temp // 10
        if(temp % 10 == 2):
        ...
        length = 0
        temp = string[string.find(search)+4:]
        for x in temp:
            if(x.isdigit()):
                length += 1
            else:
        ...
        if(length == 9):
            return True
        return False
    return int(string[string.find(search)+4 : string.find(search)+8])
...
for val in vals:
    if "byr" in val and "iyr" in val and "eyr" in val and "hgt" in val and "hcl" in val and "ecl" in val and "pid" in val:
        ...
        pid = value(val,"pid")
        test += 1
        if(byr >= 1920 and byr <= 2002) and (iyr <= 2020 and iyr >= 2010) and (eyr <= 2030 and eyr >= 2020) and pid and hcl and hgt and ecl:
            count += 1
            print(count,val)'''
        byr = int(re.search(r'byr:(\d+)',val).group(1))
        iyr = int(re.search(r'iyr:(\d+)',val).group(1))
        eyr = int(re.search(r'eyr:(\d+)',val).group(1))
        ecl = re.search(r'ecl:(#?\w+)',val).group(1)
        pid = re.search(r'#?pid:(\d+)',val).group(1)
        print(val)
        hcl = re.search(r'(hcl:#?\w+)',val).group(1)
